...
class ResBlock(nn.Module):
    def __init__(self, dim, dim_out, noise_level_emb_dim=None, dropout=0, 
                    num_heads=8, use_affine_level=False, norm_groups=32, att=True):
        super().__init__()
...
class Diffusion(nn.Module):
    ...
    def p_losses(self, x_in):
        ...
        pred_noise = self.model(torch.cat([x_in['LR'], x_noisy], dim=1), noise_level=sqrt_alpha)
...
class SR3():
    def __init__(self, device, loss_type, dataloader, testloader, schedule_opt, save_path, 
                    ...
                    img_size=64, lr=1e-5, distributed=False):
        ...
        self.sr3 = Diffusion(model, device, out_channel)
    ...
    def train(self, epoch, verbose):
        ...
        fixed_imgs['LR'] = fixed_imgs['LR'][:16].to(self.device)
        ...
        for i in tqdm(range(epoch)):
            ...
            if (i+1) % verbose == 0:
                self.sr3.eval()
                validation_loss = 0
                for _, test_imgs in enumerate(self.testloader):
                    test_imgs['HR'] = test_imgs['HR'].to(self.device)
                    test_imgs['LR'] = test_imgs['LR'].to(self.device)
                    b, c, h, w = test_imgs['HR'].shape

                    with torch.no_grad():
                        val_loss = self.sr3(test_imgs)
                        val_loss = val_loss.sum() / int(b*c*h*w)
                        validation_loss += val_loss.item() * b
                        
                self.sr3.train()

                train_loss = train_loss / len(self.dataloader)
                validation_loss = validation_loss / len(self.testloader)
                print(f'Epoch: {i+1} / loss:{train_loss:.3f} / val_loss:{validation_loss:.3f}')
                ...
                plt.imshow(np.transpose(torchvision.utils.make_grid(fixed_imgs['LR'], 
                                                                    nrow=4, padding=1, normalize=True).cpu(),(1,2,0)))

                plt.subplot(1,2,2)
                plt.axis("off")
                plt.title("Super-Resolution Results")
                plt.imshow(np.transpose(torchvision.utils.make_grid(result_SR.detach().cpu(), 
                                                                    nrow=4, padding=1, normalize=True),(1,2,0)))
...
class Dataset_SR3(Dataset):
    def __init__(self, root, transforms_HR=None, transforms_LR=None, size_limit=None):
        ...
        if size_limit is not None:
            self.total_files = self.total_files[:size_limit]
...
if __name__ == "__main__":
    ...
    testloader = DataLoader(Dataset_SR3(testroot, transforms_HR=transforms_HR, transforms_LR=transforms_LR, size_limit=100), 
    ...
    schedule_opt = {'schedule':'linear', 'n_timestep':1000, 'linear_start':1e-6, 'linear_end':1e-2}
    ...
    sr3 = SR3(device, loss_type='l1', dataloader=dataloader, testloader=testloader, schedule_opt=schedule_opt, 
                ...
                norm_groups=16, channel_mults=(1, 2, 4, 8, 8), dropout=0.2, res_blocks=2, lr=1e-4, distributed=False)
    sr3.train(epoch=200, verbose=20)
