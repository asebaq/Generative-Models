...
class Diffusion(nn.Module):
    ...
    def make_beta_schedule(self, schedule, n_timestep, linear_start=1e-4, linear_end=2e-2):
        if schedule == 'linear':
            ...
            timesteps = torch.arange(
                n_timestep + 1, dtype=torch.float64) / n_timestep + cosine_s
    ...
    def p_losses(self, x_in):
        x_start = x_in
        lr_imgs = transforms.Resize(self.img_size)(
            transforms.Resize(self.LR_size)(x_in))
...
class SR3():
    def __init__(self, device, img_size, LR_size, loss_type, dataloader, testloader,
        ...
        model = UNet(in_channel, out_channel, inner_channel,
                     norm_groups, channel_mults, res_blocks, dropout, img_size)
    ...
    def train(self, epoch, verbose):
        ...
        fixed_imgs = transforms.Resize(self.img_size)(
            transforms.Resize(self.LR_size)(fixed_imgs))
    ...
    def test(self, imgs):
        imgs_lr = transforms.Resize(self.img_size)(
            transforms.Resize(self.LR_size)(imgs))
        self.sr3.eval()
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    schedule_opt = {'schedule': 'linear', 'n_timestep': 2000,
