...
class SelfAtt(nn.Module):
    ...
    def forward(self,x):
        ...
        b, _, h, w = x.size()
        x = self.groupnorm(x)
        qkv = rearrange(self.qkv(x), "b (c qkv) h w -> (qkv) b c (h w)", qkv=3)
        queries, keys, values = qkv[0], qkv[1], qkv[2]
        head_channels = self.channel_dim // self.num_heads
        ...
        for i in range(self.num_heads):
            ...
            att_value = (context.transpose(1, 2) @ query).reshape(b, head_channels, h, w)
            att_values.append(att_value)

        att = torch.cat(att_values, dim=1)
...
class ResBlockWithAtt(nn.Module):
    def __init__(self, dim, dim_out, *, noise_level_emb_dim=None, norm_groups=32, dropout=0):
        ...
        self.attn = SelfAtt(dim_out, num_heads=1, norm_groups=norm_groups)
...
class SR3():
    ...
    def train(self, epoch, verbose):
        ...
        fixed_imgs['LR'] = fixed_imgs['LR'][:4].to(self.device)
...
if __name__ == "__main__":
    ...
    sr3 = SR3(device, loss_type='l1', dataloader=dataloader, testloader=testloader, schedule_opt=schedule_opt, 
                ...
                norm_groups=16, channel_mults=(1, 2, 4, 8, 8), dropout=0.2, res_blocks=1, lr=1e-4, distributed=False)
    sr3.train(epoch=200, verbose=10)
