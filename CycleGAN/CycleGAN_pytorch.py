...
# inum_c : Number of input image channels e.g. 3
# onum_c : Number of output image channels e.g. 3
class Generator(nn.Module):
    def __init__(self, inum_c, onum_c, n_downsample_block=2, n_residual_blocks=9):
        super(Generator, self).__init__()

        # Initial convolution block       
        model = [   nn.ReflectionPad2d(3),
                    nn.Conv2d(inum_c, 64, 7),
...
def train(n_epochs, dataloader, G_A2B, G_B2A, D_A, D_B, 
            ...
            lr_scheduler_G, lr_scheduler_D_A, lr_scheduler_D_B,cuda):
    
    # Set for using GPU if available
    ...
    for i in tqdm(range(n_epochs)):
        ...
        for _, batch in enumerate(dataloader):
            ...
            # input_A = torch.Tensor(batch_size, input_nc, size, size).to(device)
            # input_B = torch.Tensor(batch_size, output_nc, size, size).to(device)
            y_real = torch.ones(batch_size, 1).to(device)
            y_fake = torch.zeros(batch_size, 1).to(device)

            # Set model input
            # real_A = input_A.copy_(batch['A']).to(device)
            # real_B = input_B.copy_(batch['B']).to(device)
