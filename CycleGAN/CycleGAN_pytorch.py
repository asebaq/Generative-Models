...
def train(n_epochs, dataloader, G_A2B, G_B2A, D_A, D_B,
    ...
    for i in tqdm(range(n_epochs)):
        ...
        if (i + 1) % 5 == 0:
            print(
                f'Epoch: {i + 1} / loss_G:{loss_G:.4f} / loss_G_identity:{loss_identity_A + loss_identity_B:.4f} / loss_G_GAN:{loss_GAN_A2B + loss_GAN_B2A:.4f} / ',
            ...
            plt.imshow(
                np.transpose(torchvision.utils.make_grid(fixed_imgs['A'], nrow=4, padding=1, normalize=True).cpu(),
                             (1, 2, 0)))
            ...
            plt.imshow(
                np.transpose(torchvision.utils.make_grid(gen_img.detach().cpu(), nrow=4, padding=1, normalize=True),
                             (1, 2, 0)))
...
if __name__ == "__main__":
    ...
    transform = [transforms.Resize(img_size), transforms.ToTensor(),
                 transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]
    dataloader = DataLoader(Dataset_Cycle(root, transforms_=transform), batch_size=batch_size, shuffle=True,
                            num_workers=2)
    ...
    train(n_epochs=20, dataloader=dataloader, G_A2B=G_A2B, G_B2A=G_B2A, D_A=D_A, D_B=D_B,
          ...
          lr_scheduler_G=lr_scheduler_G, lr_scheduler_D_A=lr_scheduler_D_A, lr_scheduler_D_B=lr_scheduler_D_B,
          device=device)
