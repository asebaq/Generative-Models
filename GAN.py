...
class Generator(nn.Module):
    def __init__(self, img_shape, dim_latent, g_dims=[128,256,512,1024], cond=False):
...
class Discriminator(nn.Module):
    def __init__(self, img_shape, d_dims=[512, 256], cond=False):
...
def Train(epoch, dataloader, cuda, G, D, optimizer_G, optimizer_D):
    ...
    for j in tqdm(range(epoch)):
        for _, (imgs, labels) in enumerate(dataloader):
            ...
            z = Tensor(np.random.normal(0, 1, (imgs.shape[0], dim_latent)))
        ...
        print(f"Epoch {j} / D loss: {d_loss.item():.4f} / G loss: {g_loss.item():.4f}")
